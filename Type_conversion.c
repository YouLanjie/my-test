	...
	while (a != '0' && a != 'q' && a != 'Q' && a != 0x1B) {
		...
		if (a == '\r' || a == '\n' || a == '1') {
			...
			for (i = 0;
				...
				i2--;
				for (unsigned long i3 = 0; i3 <= strlen(type);
				     i3++) {
				...
				if (pid == 0) {
					printf
					    ("\033[1;33mLoading: [file]: \033[1;32m\'%s\'\033[0m\n",
					     inputF);
					fflush(stdout);
			...
			printf
			    ("\n\033[1;33mInfo: \033[1;32m耗时%2.6f秒加载完毕，开始等待\033[0m\n\n",
			     ...
			     (double)(time2.tv_usec - time.tv_usec) / 1000000);
			fflush(stdout);
			...
			printf
			    ("\n\033[1;33mInfo: \033[1;32m共%d个目标文件转换完成，耗时%2.6f秒，共耗时%2.6f秒\033[0m\n",
			     ...
			     (double)(time3.tv_usec - time2.tv_usec) / 1000000,
			     time3.tv_sec - time.tv_sec +
			     (double)(time3.tv_usec - time.tv_usec) / 1000000);
			getch_old();
		}
	}
	if (dp) {
...
int conversion(char inputF[150], char outputF[150], char dirname[100], int i)
{
	...
	if (access(outputF, 0) == 0) {
		printf
		    ("\033[1;33mWarning: [跳过已存在文件]: \033[1;32m%s\033[0m\n",
		     outputF);
		exit(1);
	}
	...
	remove(dirname);
	printf
	    ("\033[1;33m耗时约\033[1;32m%2.6f秒\033[1;33m转换完成：\033[1;32m\'%s\'\033[0m\n",
	     time2.tv_sec - time.tv_sec + (double)(time2.tv_usec -
