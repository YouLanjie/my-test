...
#+ 有冒号与后面有空格的meta格式: test1
 #+有冒号与前面有空格的meta格式: test2
#+内含 空格测试: 2
#+_下划线开头测试: 4
#+/其他字符开头测试: 5
#+*其他字符开头测试: 5
...
****** level6.0
***** level5.0.1
** 超过一定等级的标题转换成list测试2(bug test)
#+begin_quote
经测试，emacs原生html导出在处理自上而下（从level5的标题到level4）的超出h的
options设定的标题时，会出现忘记添加 =<ul>= 或者 =<ol>= 标签，导致显示错误
#+end_quote
***** level5.1
**** level4.1
内容测试
...
*** TODO COMMENT 7
*** 8[[http://baidu.com]]
*** 9[[file:///home/Chglish/Pictures/本地/编辑/2025_08_20_开学倒计时/2025_08_20_开学倒计时_16x10_withouttext.png]]
*** 10
- [[1]]
- [[2]]
- [[3]]
...
#+CAPTION: src block的caption
#+begin_src text
  使用了caption的src block
#+end_src

#+CAPTION: src block的caption+name
#+name: specialsrcblock
#+begin_src text
  使用了caption+name的src block
#+end_src
...
以上为错误内容，据最新测试，所有转义由MathJax.js自动处理，输出时保持原义
#+end_quote
** 链接
单链接测试：[[http://bing.com]]
...
http://cctv.com/测试 content

https://tv.cctv.com/2025/08/29/VIDAHV8r0zaLCzZdlz9q49DX250829.shtml
...
#+begin_quote
测试表明，只需要协议头+一定长度的有效字符即可被识别为链接,不可有回车，空格等，但
是可以有中文，
#+end_quote
*** 图片测试
图片测试1：
...
*** [[./test.jpg]]
*** 其他互相引用的链接
Dedicated targets:<<Dedicated targets>>(自我标记的链接)

很明显，它不会显示

引用:[[Dedicated targets]]; [[Dedicated targets][引用]]

Radio targets(全局自动产生链接):\\
<<<史莱姆>>>，一个汉语词汇(这个到会显示)
...
引用pre的NAME:[[specialsrcblock]]

[[http://bilibili.com][h<<<B站>>>e]](嵌套否?)

B站，B站NB

显然不可

<<<tall>>>,tall,taller,tall1,tall点
...
#+end_src
*** 进阶的 /破坏性格式/ 测试
[2025-01-01 其他奇奇怪怪的东西]

[2025-01-02 其他奇奇怪怪的东西 07:21]

[2025-01-03 07:21 其他奇奇怪怪的东西]

[2025-01-04 fasdfaweqr07:21 其他奇奇怪怪的东西]

[2025-01-05 fasdfaweqr 07:21其他奇奇怪怪的东西]

[2025-01-06 fasdfaweqr 07:21 其他奇奇怪怪的东西]

[ 2025-01-06 07:21]

[2025-01-07 1:12 10:1 07:21 其他奇奇怪怪的东西]

[2025-01-08 10:1 07:21]

[2025-01-09 10:-1 07:21]
...
#+begin_quote
总结：只有在至少[YYYY-MM-DD]或<YYYY-MM-DD>的格式才能触发timestamp,例如
<2025-10-01 三>,Emacs使用 =C-c .= 进行插入。<2025-10-01 三 13:03>

在导出时只识别日期和有效时间，无效信息会被丢弃，错误信息会被修正。如星期会被修正，
超出正常时间的时分秒会被“进位”到正确的时间，如30:21会被修正为第二天早上7:21，
精确到秒的时间会被丢弃到分，不完整的仅有小时会被丢弃

时间的识别应该是识别到日期后面的字符串中第一个数字并匹配查看是否满足1位或2位小时
数字的要求，冒号后为2位分钟数字，如果匹配出错则不再匹配。

导出时使用 =[]= 还是 =<>= 取决于开头的符号，这意味着两端符号混用也是没有问题的。
#+end_quote
** 对特殊字符的处理
示例： =0a`b~c!d@e#f$g%h^i&j*k(l)m_n-o+p=q[r]s{t}u;v:w'x"y\z|a<b,c>d.e?f g.jpg=

- 链接：[[./0a`b~c!d@e#f$g%h^i&j*k(l)m_n-o+p=q\[r\]s{t}u;v:w'x"y\z|a<b,c>d.e?f g.jpg][链接源]] [[./0a`b~c!d@e#f$g%h^i&j*k(l)m_n-o+p=q\[r\]s{t}u;v:w'x"y\z|a<b,c>d.e?f g.jpg][DESC:0a`b~c!d@e#f$g%h^i&j*k(l)m_n-o+p=q[r]s{t}u;v:w'x"y\z|a<b,c>d.e?f g]]
- <i>防html直接插入</i>
** 其他
原内容：
#+begin_src text
