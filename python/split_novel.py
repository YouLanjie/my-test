...
def seperate_str(pattern: re.Pattern, content: str):
    ...
    for match in pattern.finditer(content):
        ...
        skip = 1
        new_key = headline
        while new_key in groups and headline:
            new_key = tuple(list(headline[:-1])+[headline[-1]+f"_fsk{skip}"])
            skip += 1
        headline = new_key
        if last_match:
...
class Tree:
    ...
    def mkdir(self, path: list|tuple) -> None|dict:
        ...
        for i in path:
            key = i
            skip = 1
            while key in cwd and self.last_path[:len(new_path+[key])] != new_path+[key]:
                key = f"{i}_sk{skip}"
                skip += 1
            if key not in cwd:
                cwd[key] = {}
    ...
    def get(self, path: list|tuple) -> None|dict:
        ...
        for i in path:
            ...
            cwd = cwd[i]
        return cwd
    def get_id_list(self, node:dict, prefix:None|list=None):
    ...
    def squash(self, cwd=None, sep=" ") -> None:
        ...
        for k,v in cwd.items():
            if not isinstance(v, dict):
                new_cwd[k] = v
                continue
            self.squash(cwd=v, sep=sep)
            new_k = fix_pattern.sub("", k)+sep+list(v)[0] if len(v)>0 else k
            if (len(v) == 1) and new_k not in cwd:
                new_cwd[new_k] = v[list(v)[0]]
            else:
                new_cwd[k] = v
        cwd.clear()
        for k,v in new_cwd.items():
            cwd[k] = v
    def to_list(self, begin=0):
        """将树状结构恢复为list"""
        split="\0\n\0/#/#/\0\n\0"
        tree = pytools.squash_dict(self.tree, split=split)
        li : list[tuple[list, str]] = [(k.split(split),v) for k,v in tree.items()]
        return li[begin:]

def to_tree(groups:dict[tuple, list[str]], blacklist:re.Pattern, print_func=pytools.print_err):
    """词典转成词典树"""
    tree = Tree()
    for k,v in groups.items():
        tree.touch(k, content=v)
        # tree.touch(k, content=[str(len(v))])
    tree.squash()
    tree.squash()
    tree.squash()
    # import pprint
    # pprint.pprint(tree.tree, sort_dicts=False)
    ...
    for path, s in tree.to_list(1):
        ...
        while same_len < len(path):
            ...
            if not blacklist.search(headline):
                ret[(file, count)].append(f"{"*"*(same_len+1)} {headline}")
                print_func(f"{" "*(same_len)}- {headline}")
            same_len+=1
        ret[(file, count)] += s
        last = path
    return ret
...
def main():
    ...
    for w1,w2 in cfg["words"]:
        pytools.print_err(f"替换 {repr(w1)} 为 {repr(w2)} : 共计{len(re.findall(w1, content))}处")
    ...
    docs = to_tree(seperate_str(pattern, content), blacklist=blacklist,
                   print_func=pytools.print_err if args.simple else print)
    ...
    for h1,content in docs.items():
        ind += 1
        outf = Path(f"{ind:03d}_{h1[0]}.org")
        pytools.print_err(f"- 输出到文件: {outf}")
        if outf.is_dir():
            continue
        s = "\n\n".join([i.strip() if cfg["strip"] else i for i in content if i])
        if not args.dry_run:
