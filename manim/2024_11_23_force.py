...
class force(Scene):
    ...
    def scene_1(self, st):
        ...
        self.play(Transform(arrow, Arrow(DOWN*2, ORIGIN)))
        self.play(Transform(arrow, Arrow(DOWN*2, UP*4)))
        self.wait(0.3)
    ...
    def scene_2(self, st):
        arrow1 = Arrow([-4,0,0], [-2,0,0])
        arrow2 = Arrow([-4,0,0], [0,0,0])
        arrow3 = Arrow([-4,0,0], [4,0,0])
        self.play(Create(arrow1))
        self.sub_text(st, "在物理中，位移，速度和力等物理量都是矢量", 0.5)
        self.sub_text(st, "而矢量的运算法则大致如下", 0.5)
        self.play(Transform(arrow2, Arrow([-4,2,0], [0,2,0])))
        self.play(Transform(arrow3, Arrow([-4,-2,0], [0,-2,0])))
        # arrow4 = Arrow([-4, 2, 0], [-2, 2, 0])
        # arrow5 = Arrow([-4, 2, 0], [2, -2, 0])
    ...
    def construct(self):
        ...
        st = Text("这是一个矢量").set_color(RED).shift(3*DOWN)
        # self.scene_1(st)
        self.scene_2(st)
        self.wait(2)
        pass
