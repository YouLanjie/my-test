...
move() {
	...
	#echo $file_list
	file_list=$(find $input/ -maxdepth 1 -name '*.mp4')
	max_line=$(echo $file_list|wc -l)
	for (( i=1; i <= $max_line; i++)) {
		name=$(echo $file_list|sed -n "${i}p")
		out=$(echo $name|sed -n "s/$input\(.*\)\([0-9]\{4\}\)-\([0-9]\{2\}\)-\([0-9]\{2\}\)\( .*\)/$output\1\2\/\2_\3\/\2_\3_\4\/\1\2-\3-\4\5/p")
		if [[ $out == "" ]] {
			continue
		}
		echo ""
		#out_d=$(echo $name|sed -n "s/\(Screenshot_\)\([0-9]\{4\}\)\([0-9]\{2\}\)\([0-9]\{2\}\)\(_.*\)/\2\/\2_\3\/\2_\3_\4/p")
		out_d=$(echo $name|sed -n "s/$input\(.*\)\([0-9]\{4\}\)-\([0-9]\{2\}\)-\([0-9]\{2\}\)\( .*\)/$output\1\2\/\2_\3\/\2_\3_\4/p")
		#                      sed -n "s/\.\(.*\)\([0-9]\{4\}\)-\([0-9]\{2\}\)-\([0-9]\{2\}\)\( .*\)/.      \1\2\/\2_\3\/\2_\3_\4\/\1\2-\3-\4\5/p"
		if [[ ! -d $out_d ]] {
